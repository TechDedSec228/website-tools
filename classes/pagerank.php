...
class get_pagerank {
  ...
  # functions added to get pagerank pagerank
  function get_assigned_pagerank($domain) {
	global $data_grab;
	...
	$url = "http://www.google.com/search?client=navclient-auto&ch=".$this->CheckHash($this->HashURL($domain))."&features=Rank&q=info:".$domain."&num=100&filter=0";
	  
//	echo $url;
//	$data_grab->link_checking = 0;
	$data_grab->attemp = 1;
	$data_grab->get_page_data($url);
	$html_grab = $data_grab->html_grab;
  
	preg_match('/Rank_([0-9]+):([0-9]+):([0-9]+)/si', $html_grab, $r);
  return ($r[3]) ? $r[3] : '0';
  }  
